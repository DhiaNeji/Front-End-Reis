...
@Component({
  selector: 'app-correction-registre-commercial',
  templateUrl: './correction-registre-commercial.component.html',
  styleUrls: ['./correction-registre-commercial.component.scss']
})
export class CorrectionRegistreCommercialComponent implements OnInit {
  ngOnInit(){
  }
  public options2
  ...
  constructor(private toastr:ToastrService,private service:ServiceService) {
    ...
    this.service.getProgcess("5").subscribe(res=>{
  ...
  dataSource:IServerSideDatasource={
    getRows:(params:IServerSideGetRowsParams)=>{
      this.service.get_data("5",params.request.startRow,params.request.endRow).subscribe(response=>{
  ...
  edit(event)
  {
    ...
    this.service.update_data("5",event.data,event.rowIndex).subscribe(res=>{
...
rowSelectedHandler($event)
{
  ...
  this.gridApi.getRowNode($event.data.id).setDataValue('step_5','false')
}
prepareData(){
  ...
  this.service.prepare_data("7").then(data=>{
  this.toastr.clear()
