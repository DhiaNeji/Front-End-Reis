...
@Component({
  selector: 'app-correction-matricule-fiscale',
  templateUrl: './correction-matricule-fiscale.component.html',
  styleUrls: ['./correction-matricule-fiscale.component.scss']
})
export class CorrectionMatriculeFiscaleComponent implements OnInit {
  ngOnInit(){
  }
  ...
  constructor(private toastr:ToastrService,private service:ServiceService) {
    ...
    this.service.getProgcess("4").subscribe(res=>{
  ...
  dataSource:IServerSideDatasource={
    getRows:(params:IServerSideGetRowsParams)=>{
      this.service.get_data("4",params.request.startRow,params.request.endRow).subscribe(response=>{
  ...
  edit(event)
  {
    ...
    this.service.update_data("4",event.data,event.rowIndex).subscribe(res=>{
...
rowSelectedHandler($event)
{
  ...
  this.gridApi.getRowNode($event.data.id).setDataValue('step_4','false')
}
prepareData(){
  ...
  this.service.prepare_data("10").then(data=>{
  this.toastr.clear()
