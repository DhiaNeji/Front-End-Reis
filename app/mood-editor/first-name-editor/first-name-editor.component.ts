...
import { FirstNameValidatorComponent } from '../first-name-validator/first-name-validator.component';

@Component({
  selector: 'app-first-name-editor',
  template: `
   <input
      #input
...
export class FirstNameEditorComponent implements  ICellEditorAngularComp, AfterViewInit  {
  private params: any;
  public value: number;
  ...
  onClick()
  {
    if(this.validateInput(this.value))
    {
      ...
      this.service.validateNom(this.value).subscribe(res=>{

        this.params.data.first_name=this.value
        this.params.api.getRowNode(this.params.data.id).setDataValue('step_10','true')
        this.params.api.stopEditing();
        this.params.api.hideOverlay()
      },err=>{
        this.params.api.hideOverlay()
      const dialogRef = this.dialog.open(FirstNameValidatorComponent, {
        width: '350px',
        data: this.value
      });
