...
@Component({
  selector: 'app-correction-numero-tel',
  templateUrl: './correction-numero-tel.component.html',
  styleUrls: ['./correction-numero-tel.component.scss']
})
export class CorrectionNumeroTelComponent implements OnInit {
  ngOnInit(){
  }
  public options2
  ...
  constructor(private toastr:ToastrService,private service:ServiceService) {
    ...
    this.service.getProgcess("12").subscribe(res=>{
    ...
    this.columnDefs = [
      ...
      {
      field:'Action',
      cellRenderer: 'ValidateTelComponent',
      }
    ];
    this.defaultColDef = {
      flex: 1,
  ...
  this.frameworkComponents = {
    ValidateTelComponent:ValidateTelComponent
  };
  ...
  this.getRowStyle= function(params) {
    if(params.data!=null)
   {
     console.log(params.data.step_12)
    if (params.data.step_12=='false') {
        return { 'pointer-events': 'none', opacity: '1',background: '#FFEEBC' }
    }
    else
    {
       if(params.data.step_12==null)
  ...
  dataSource:IServerSideDatasource={
    getRows:(params:IServerSideGetRowsParams)=>{
      this.service.get_data("12",params.request.startRow,params.request.endRow).subscribe(response=>{
  ...
  edit(event)
  {
    if(event.data.step_12!=null)
    {
    this.gridApi.showLoadingOverlay()
    this.toastr.info("Mise à jour en cours")
    this.service.update_data("12",event.data,event.rowIndex).subscribe(res=>{
...
rowSelectedHandler($event)
{
  ...
  this.gridApi.getRowNode($event.data.id).setDataValue('step_12','false')
}
prepareData(){
...
ValidationCellEditor.prototype.isValid = function(value) {
      var Regex=RegExp('^[2,3,5,9]{1}[0-9]{7}$')
