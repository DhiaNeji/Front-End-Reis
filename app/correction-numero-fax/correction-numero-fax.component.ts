...
@Component({
  selector: 'app-correction-numero-fax',
  templateUrl: './correction-numero-fax.component.html',
  styleUrls: ['./correction-numero-fax.component.scss']
})
export class CorrectionNumeroFaxComponent implements OnInit {
  ngOnInit(){
  }
  public options2
  ...
  constructor(private toastr:ToastrService,private service:ServiceService) {
    ...
    this.service.getProgcess("13").subscribe(res=>{
    ...
    this.columnDefs = [
      ...
      {
      field:'Action',
      cellRenderer: 'ValidateFaxComponent',
      }
    ...
    this.frameworkComponents = {
      ValidateFaxComponent:ValidateFaxComponent
    };
  ...
  this.getRowStyle= function(params) {
    if(params.data!=null)
   {
    if (params.data.step_13=='false') {
        return { 'pointer-events': 'none', opacity: '1',background: '#FFEEBC' }
    }
    else
    {
       if(params.data.step_13==null)
  ...
  dataSource:IServerSideDatasource={
    getRows:(params:IServerSideGetRowsParams)=>{
      this.service.get_data("13",params.request.startRow,params.request.endRow).subscribe(response=>{
  ...
  edit(event)
  {
    if(event.data.step_13!=null)
    {
    this.gridApi.showLoadingOverlay()
    this.toastr.info("Mise à jour en cours")
    this.service.update_data("13",event.data,event.rowIndex).subscribe(res=>{
...
rowSelectedHandler($event)
{
  ...
  this.gridApi.getRowNode($event.data.id).setDataValue('step_13','false')
}
prepareData(){
