...
@Component({
  selector: 'app-correction-prenom',
  templateUrl: './correction-prenom.component.html',
  styleUrls: ['./correction-prenom.component.scss']
})
export class CorrectionPrenomComponent implements OnInit {
  ngOnInit(){
  }
  public options2
  ...
  constructor(private toastr:ToastrService,private service:ServiceService) {
    ...
    this.service.getProgcess("11").subscribe(res=>{
    ...
    this.columnDefs = [
      ...
      },
      {
        field: 'last_name',
        headerName:'Prenom',
        cellEditor:'LastNameEditorComponent',
        editable:true,
      },
      {
        field: 'birthdate',
  ...
  this.getRowStyle= function(params) {
    if(params.data!=null)
   {
    if (params.data.step_11=='false') {
        return { 'pointer-events': 'none', opacity: '1',background: '#FFEEBC' }
    }
    else
    {
       if(params.data.step_11=='true')
  ...
  dataSource:IServerSideDatasource={
    getRows:(params:IServerSideGetRowsParams)=>{
      this.service.get_data("11",params.request.startRow,params.request.endRow).subscribe(response=>{
  ...
  edit(event)
  {
    ...
    this.service.update_data("11",event.data,event.rowIndex).subscribe(res=>{
...
rowSelectedHandler($event)
{
  ...
  this.gridApi.getRowNode($event.data.id).setDataValue('step_11','false')
}
prepareData(){
