...
@Component({
  selector: 'app-correction-nid',
  templateUrl: './correction-nid.component.html',
  styleUrls: ['./correction-nid.component.scss']
})
export class CorrectionNidComponent implements OnInit {
  ngOnInit(){
  }
  public options2
  ...
  constructor(private toastr:ToastrService,private service:ServiceService) {
    ...
    this.service.getProgcess("1").subscribe(res=>{
  ...
  dataSource:IServerSideDatasource={
    getRows:(params:IServerSideGetRowsParams)=>{
      this.service.get_data("1",params.request.startRow,params.request.endRow).subscribe(response=>{
  ...
  edit(event)
  {
    ...
    this.service.update_data("1",event.data,event.rowIndex).subscribe(res=>{
...
rowSelectedHandler($event)
{
  ...
  this.gridApi.getRowNode($event.data.id).setDataValue('step_1','false')
}
prepareData(){
